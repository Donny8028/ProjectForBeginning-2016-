...
class GPX: NSObject, NSXMLParserDelegate
{
    ...
    private init(url: NSURL, completionHandler: GPXCompletionHandler) {
        self.url = url
        self.completionHandler = completionHandler // 3. Just initializing GPX file and completionHandler, not yet implement it.
    }
    
    private func complete(success success: Bool) {   // 4. create a Boolean method to decide if it succeed
        dispatch_async(dispatch_get_main_queue()) {  // when succeed, back the main queue.
            self.completionHandler(success ? self : nil)
        }
    }
    
    private func fail() { complete(success: false) }
    private func succeed() { complete(success: true) }  // 5. implement it
    
    private func parse() {                                   //1. when initializer begins here
        let qos = Int(QOS_CLASS_USER_INITIATED.rawValue)
        dispatch_async(dispatch_get_global_queue(qos, 0)) {
            if let data = NSData(contentsOfURL: self.url) {  //block the thread here
    ...
    func parserDidEndDocument(parser: NSXMLParser) { succeed() } // 2. parse succeed
    ...
    func parser(parser: NSXMLParser, didStartElement elementName: String, namespaceURI: String?, qualifiedName qName: String?, attributes attributeDict: [String : String]) {
        switch elementName {
            ...
            case "rtept", "trkpt", "wpt":
                ...
                waypoint = Waypoint(latitude: latitude, longitude: longitude)    //Waypoint class got initialized
            case "link":
                link = Link(href: attributeDict["href"]! as String)              //Link class got initialized
            default: break
        }
    }
